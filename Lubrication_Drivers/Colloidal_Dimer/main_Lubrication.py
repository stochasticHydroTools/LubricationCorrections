...
if __name__ == '__main__':
    ...
    debye_delta = 1e-2
    debye_length_delta = 2.0*a*debye_delta/np.log(1.0e1)
    repulsion_strength_delta = read.repulsion_strength_wall
    
    LSolv = LS(bodies,a,eta,cutoff,L,debye_length=debye_delta)
    LSolv.dt = dt
    LSolv.kT = read.kT
    ...
    for n in range(n_steps):
      ...
      FT_calc = partial(multi_bodies_functions.force_torque_calculator_sort_by_bodies, 
                                               g = read.g, 
                                               repulsion_strength_wall = repulsion_strength_delta,
                                               debye_length_wall = debye_length_delta, 
                                               debye_delta = debye_delta,
      ...
      dist = np.linalg.norm(bodies[0].location-bodies[1].location)
      height = min(bodies[0].location[2],bodies[1].location[2])
      print("Particle eps : %.8f" %(dist-2*a))
      print("Particle height eps : %.8f" %(height-a))
